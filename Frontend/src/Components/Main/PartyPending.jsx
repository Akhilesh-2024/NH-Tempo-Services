...
const PartyPending = () => {
  ...
  const filteredPendingBookings = useMemo(() => {
    return bookings.filter((b) => {
      const isPending =
        ...
        b.paymentStatus?.partyPaymentStatus === "partial";
  ...
  const handleSelectBookingForPayment = (booking) => {
    if (booking) {
      ...
      const pendingAmount = Math.max(0, totals.finalPendingAmount);
  ...
  const handlePaymentSubmit = async (e) => {
    ...
    try {
      await dispatch(
        addPartyPayment({ id: paymentData.bookingId, paymentData })
      ).unwrap();
  ...
  const exportToCSV = () => {
    const csvContent = [
      ...
      ...filteredPendingBookings.map((b) => {
        ...
        return [
          ...
          totals.dealAmount,
          totals.advancePaid,
          Math.max(0, totals.finalPendingAmount),
        ].join(",");
      }),
    ].join("\n");
    ...
    link.setAttribute("download", "pending_party_payments.csv");
  ...
  const exportAllHistoryToCSV = () => {
    const csvContent = [
      ["Booking No", "Party Name", "Date", "Mode", "Amount", "Remarks"].join(
        ","
      ),
      ...bookings.flatMap((b) =>
        (b.charges?.paymentHistory || []).map((p) =>
          [
            b.bookingNo,
            `"${b.party?.name || ""}"`,
    ...
    const link = document.createElement("a");
    link.href = URL.createObjectURL(blob);
    link.setAttribute("download", "all_payment_history.csv");
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  };
      ...
      <div className="max-w-screen-2xl mx-auto">
        ...
        <div className="bg-gray-800 p-6 rounded-2xl shadow-lg border border-gray-700 mb-6">
          <h2 className="text-xl font-bold text-white mb-4 flex items-center gap-2">
            <FaMoneyBillWave /> Add Party Payment
          </h2>
          <form
          ...
          <div className="p-4 border-b border-gray-700 space-y-4">
            <div className="flex justify-between items-center">
              <h2 className="text-xl font-bold text-white">
                Pending Party Payments ({filteredPendingBookings.length})
              </h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Search by Booking No or Party..."
                  value={searchTerm}
                ...
                {filteredPendingBookings.map((booking) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-4 text-gray-300">
                        ₹{totals.dealAmount.toLocaleString("en-IN")}
                      </td>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
          <div className="bg-gray-800 rounded-lg shadow-xl w-full max-w-md">
            ...
            <div className="p-4 max-h-96 overflow-y-auto">
              {selectedBookingForHistory.charges?.paymentHistory?.length > 0 ? (
                <ul className="space-y-3">
                  {selectedBookingForHistory.charges.paymentHistory.map(
                    (p, index) => (
                      <li
                        key={index}
      ...
      {isAllHistoryModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
          <div className="bg-gray-800 rounded-lg shadow-xl w-full max-w-3xl max-h-[80vh] overflow-y-auto">
            <div className="p-4 border-b border-gray-700 flex justify-between items-center">
              <h3 className="text-lg font-bold">All Payment History</h3>
              <div className="flex gap-3">
                <button
            ...
            <div className="p-4 space-y-6">
              {bookings.filter((b) => b.charges?.paymentHistory?.length > 0)
                .length > 0 ? (
                bookings
                  .filter((b) => b.charges?.paymentHistory?.length > 0)
                  .map((b) => (
                    <div key={b._id} className="bg-gray-700 p-4 rounded-lg">
                      <h4 className="text-white font-bold mb-2">
                        {b.bookingNo} - {b.party?.name}
                      </h4>
                      <ul className="space-y-2 text-sm">
                        {b.charges.paymentHistory.map((p, idx) => (
                          <li
                            key={idx}
